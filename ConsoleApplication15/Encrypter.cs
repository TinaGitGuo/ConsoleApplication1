...
namespace EncriptSample
{
    ...
    class Encrypter
    {
        ...
        public static string HashAndSignString2(string plaintext, string privateKey)
        {
            UnicodeEncoding ByteConverter = new UnicodeEncoding();
            byte[] dataToEncrypt = ByteConverter.GetBytes(plaintext);
            ...
            //using (RSACryptoServiceProvider RSAalg = (new X509Certificate2()).PrivateKey as RSACryptoServiceProvider )
            //{
            // //   RSAalg.FromXmlString((new X509Certificate2()).PrivateKey.ToString());  // privateKey =cert.PrivateKey  or    cert.PrivateKey as  RSACryptoServiceProvider
            //    //Using SHA1 to calculate the summary, generating a signature
            //    byte[] encryptedData = RSAalg.SignData("Fill in the bit type of your file,You can use this to convert( ByteConverter.GetBytes(plaintext);)", new SHA1CryptoServiceProvider());
            //    return Convert.ToBase64String(encryptedData);
